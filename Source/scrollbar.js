var Scrollbar = new Class({
    ...
    setup: function() {
        ...
        elts.contentInner.set('html', html)
            .setStyle('padding', originalPadding)
            .inject(elts.content);
        elts.content.inject(this.element);
        this.elements = elts;

        // Scroll fx
        this.contentFx = new Fx.Tween(elts.content, {
            property: 'top',
            link: 'cancel',
            duration: 'short'
        });
        ...
        new Drag(elts.button, {
            ...
            onDrag: function(button, e) {
                this.moveTo(-(button.getStyle('top')
                    .toInt() / s.scrollbarRatio));
            }.bind(this)
        });
    ...
    update: function() {
        ...
        var s = {
            content: elts.content.getSize()
                .y,
            element: this.element.getSize()
                .y,
            scrollbar: elts.scrollbar.getSize()
                .y,
            tArrow: elts.tArrow.getSize()
                .y,
            bArrow: elts.bArrow.getSize()
                .y
        };
    ...
    /**
     * Move to animated (x) pixels
     * x must be negative
     */
    moveToAnimated: function(top, updateButtonPosition) {
        var sizes = this.sizes,
            elts = this.elements;
        top = [top, 0].pick()
            .limit(sizes.maxTop, 0);
        if (updateButtonPosition) {
            var buttonTop = -top * sizes.scrollbarRatio;
            elts.button.tween('top', buttonTop);
        }
        elts.content.tween('top', top);
        this.top = top.toInt();
        return this;
    },
    /**
    ...
    /**
     * Move of animated (x) pixels
     * x can be positive or negative
     */
    moveOfAnimated: function(x, updateButtonPosition) {
        var distance = [distance, 0].pick(),
            newTop = this.top + x;
        this.moveToAnimated(newTop, updateButtonPosition);
        return this;
    }
});
